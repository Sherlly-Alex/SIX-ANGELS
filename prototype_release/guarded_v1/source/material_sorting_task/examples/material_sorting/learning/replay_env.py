...
def load_replay_dataset(path: str | Path) -> tuple[dict[str, Any], ...]:
    ...
    for line_number, line in enumerate(
        ...
        outcome_probabilities = value.get("candidate_outcome_probabilities")
        if outcome_probabilities is not None:
            if not isinstance(outcome_probabilities, list) or len(
                outcome_probabilities
            ) != maximum:
                raise ValueError(
                    f"line {line_number}: outcome labels have invalid shape"
                )
            for index, allowed in enumerate(mask):
                probability = outcome_probabilities[index]
                if not allowed:
                    if probability is not None:
                        raise ValueError(
                            f"line {line_number}: masked slot exposes outcome truth"
                        )
                    continue
                try:
                    valid_probability = (
                ...
                if not valid_probability:
                    raise ValueError(
                        f"line {line_number}: enabled slot lacks outcome truth"
                    )
        potential_successes = value.get("candidate_potential_successes")
        if potential_successes is not None and (
            not isinstance(potential_successes, list)
            or len(potential_successes) != maximum
            or any(item not in {True, False, None} for item in potential_successes)
        ):
            raise ValueError(
                f"line {line_number}: potential outcomes have invalid shape"
            )
        record = dict(value)
...
class ReplayBanditEnv(_EnvBase):
    ...
    def __init__(
        ...
        *,
        ...
        outcome_success_weight: float = 0.0,
        time_penalty_weight: float = 0.0,
        path_penalty_weight: float = 0.0,
    ) -> None:
        ...
        for name, value in (
            ...
            ("outcome_success_weight", outcome_success_weight),
            ("time_penalty_weight", time_penalty_weight),
            ("path_penalty_weight", path_penalty_weight),
        ):
            if not math.isfinite(float(value)):
                raise ValueError(f"{name} must be finite")
        if utility_regret_scale < 0.0:
            raise ValueError("utility_regret_scale must be non-negative")
        if time_penalty_weight < 0.0:
            raise ValueError("time_penalty_weight must be non-negative")
        if path_penalty_weight < 0.0:
            raise ValueError("path_penalty_weight must be non-negative")
        ...
        self.outcome_success_weight = float(outcome_success_weight)
        self.time_penalty_weight = float(time_penalty_weight)
        self.path_penalty_weight = float(path_penalty_weight)
        self._randomizer = (
            None
    ...
    def _materialize(self, source: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if source.get("candidate_outcome_probabilities") is not None:
            record["candidate_outcome_probabilities"] = list(
                source["candidate_outcome_probabilities"]
            )
    ...
    def step(self, action: int):
        ...
        if valid_action:
            ...
            chosen_utility = float(utilities[chosen_index])
            best_utility = max(
                float(value)
            ...
            observation = np.asarray(self._current["observation"], dtype=np.float32)
            width = len(CANDIDATE_FEATURE_NAMES)
            base = len(GLOBAL_FEATURE_NAMES)
            success_index = CANDIDATE_FEATURE_NAMES.index("success_probability")
            time_index = CANDIDATE_FEATURE_NAMES.index("expected_time_s")
            path_index = CANDIDATE_FEATURE_NAMES.index("path_length_m")
            chosen_success = float(observation[base + chosen_index * width + success_index])
            chosen_time = float(observation[base + chosen_index * width + time_index])
            chosen_path = float(observation[base + chosen_index * width + path_index])
            outcome_probabilities = self._current.get(
                "candidate_outcome_probabilities"
            )
            outcome_success = (
                chosen_success
                if outcome_probabilities is None
                else float(outcome_probabilities[chosen_index])
            )
            reward = (
                self.best_reward
                - self.utility_regret_scale * utility_regret
                + self.success_weight * chosen_success
                + self.outcome_success_weight * outcome_success
                - self.time_penalty_weight * chosen_time
                - self.path_penalty_weight * chosen_path
            )
        else:
...
def build_replay_env() -> ReplayBanditEnv:
    ...
    return ReplayBanditEnv(
        ...
        success_weight=float(reward.get("success_weight", 0.0)),
        outcome_success_weight=float(
            reward.get("outcome_success_weight", 0.0)
        ),
        time_penalty_weight=float(reward.get("time_penalty_weight", 0.0)),
        path_penalty_weight=float(reward.get("path_penalty_weight", 0.0)),
    )
