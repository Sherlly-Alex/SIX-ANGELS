...
class OpenPregraspController:
    ...
    def __init__(
        ...
        *,
        ...
        pregrasp_half_width_m: float | None = None,
    ) -> None:
        ...
        self._pregrasp_half_width_m = float(
            BOX_WIDTH_Y * 0.5 + SIDE_CLEARANCE
            if pregrasp_half_width_m is None
            else pregrasp_half_width_m
        )
        if (
            not math.isfinite(self._pregrasp_half_width_m)
            or self._pregrasp_half_width_m <= 0.0
        ):
            raise ValueError("pregrasp_half_width_m must be finite and positive")
    ...
    @property
    def pregrasp_half_width_m(self) -> float:
        return self._pregrasp_half_width_m
    ...
    def plan(
        ...
        return self._plan_pose(
            ...
            half_width=self._pregrasp_half_width_m,
        )
...
class ContactGraspController(OpenPregraspController):
    ...
    def __init__(
        ...
        kdl: MMK2Kdl | None = None,
        *,
        center_backoff_x: float | None = None,
    ) -> None:
        super().__init__(kdl=kdl)
        self._center_backoff_x = float(
            GRASP_BACKOFF_X if center_backoff_x is None else center_backoff_x
        )
        if not math.isfinite(self._center_backoff_x):
            raise ValueError("center_backoff_x must be finite")
    ...
    @property
    def center_backoff_x(self) -> float:
        return self._center_backoff_x
    ...
    def plan(
        ...
        command = self._plan_pose(
            ...
            odometry,
            joint_states,
            center_backoff_x=self._center_backoff_x,
    ...
    def _update_wrist_compliance(
        ...
        *,
        ...
        opposite_wrist = (
            self._right_wrist
            if wrist is self._left_wrist
            else self._left_wrist
        )
        # Once one pad is aligned against the box, the resulting physical
        # constraint can keep the *combined* dual-arm pose from ever becoming
        # settled while the opposite pad closes the remaining gap.  Requiring
        # pose_settled in that state discarded genuine opposite-side contact
        # (high sustained effort, visible wrist rotation and near-zero wrist
        # speed), so the arms continued chasing the world target until IK left
        # its workspace.  Admit only this much stronger one-side-locked signal;
        # the normal debounce and alignment debounce below still apply before
        # bilateral grasp completion can be reported.
        opposite_side_locked_contact = (
            opposite_wrist.aligned
            and wrist.latest_effort_delta
            >= WRIST_FIRM_CONTACT_EFFORT_DELTA
            and wrist.latest_angle_delta
            >= WRIST_CONTACT_MIN_ROTATION_RAD
        ...
        contact_evidence = effort_evidence and (
            ...
            or opposite_side_locked_contact
        )
