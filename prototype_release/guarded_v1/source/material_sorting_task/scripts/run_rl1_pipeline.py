...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--training-only-dataset", type=Path)
    ...
    training_only_dataset = (
        args.training_only_dataset.resolve()
        if args.training_only_dataset is not None
        else None
    )
    output = args.output_dir.resolve()
    if not dataset.is_file():
        raise SystemExit(f"dataset does not exist: {dataset}")
    if training_only_dataset is not None and not training_only_dataset.is_file():
        raise SystemExit(
            f"training-only dataset does not exist: {training_only_dataset}"
        )
    ...
    try:
        split_command = [
            sys.executable,
            str(ROOT / "scripts" / "split_replay_dataset.py"),
            "--dataset", str(dataset),
            "--output-dir", str(split_dir),
            "--seed", str(args.seed),
            "--train-sessions", str(args.train_sessions),
            "--validation-sessions", str(args.validation_sessions),
            "--test-sessions", str(args.test_sessions),
        ]
        ...
        payload = {"passed": bool(heldout.get("passed")), "failures": heldout.get("failures", []),
                   ...
                   "training_only_dataset_sha256": (
                       _sha256(training_only_dataset)
                       if training_only_dataset is not None
                       else None
                   ),
