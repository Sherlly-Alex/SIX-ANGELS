...
def test_training_only_dataset_is_appended_only_to_train() -> None:
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        primary = _dataset(root / "primary", sessions=5)
        auxiliary = _dataset(root / "auxiliary", sessions=3)
        auxiliary_records = [
            json.loads(line)
            for line in auxiliary.read_text(encoding="utf-8").splitlines()
        ]
        for index, record in enumerate(auxiliary_records, 101):
            record["source_sha256"] = f"{index:064x}"
        ...
        result = split_replay_dataset(
            primary,
            root / "split",
            training_only_dataset_path=auxiliary,
            seed=7,
        )

        auxiliary_sessions = {
            (record["source_sha256"], record["session_index"])
            for record in auxiliary_records
        }
        manifest_sessions = {
            name: {
                (item["source_sha256"], item["session_index"])
                for item in result.manifest["sessions"][name]
            }
            for name in ("train", "validation", "test")
        }
        assert auxiliary_sessions <= manifest_sessions["train"]
        assert not auxiliary_sessions & manifest_sessions["validation"]
        assert not auxiliary_sessions & manifest_sessions["test"]
        assert result.manifest["training_only_input"]["session_count"] == 3
        assert result.manifest["session_counts"] == {
            "train": 6,
            "validation": 1,
            "test": 1,
...
def test_pipeline_forwards_requested_session_counts(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        dataset = _dataset(root / "source", sessions=10)
        captured: list[str] = []

        def capture_step(command, *, env, output):
            del env, output
            captured.extend(command)
            raise RuntimeError("stop after split command capture")
        ...
        assert captured[captured.index("--train-sessions") + 1] == "6"
        assert captured[captured.index("--validation-sessions") + 1] == "2"
        assert captured[captured.index("--test-sessions") + 1] == "2"
