...
class Evaluation:
    ...
    irreversible_risk: float | None = None
...
def test_consecutive_policy_errors_quarantine_guarded_policy() -> None:
    guard = PolicyGuard(
        PolicyGuardConfig(
            inference_timeout_s=0.2,
    ...
    try:
        for expected in ("policy_error", "policy_error"):
            result = guard.select(
                ErrorPolicy(),
                np.zeros(2),
                candidates=_candidates(),
        ...
        assert third.reason == "policy_error"
        assert guard.quarantined
        ...
        assert isolated.reason == "policy_error_quarantined"
        assert isolated.selected.action_id == "heuristic"
    finally:
        guard.close()


def test_lower_success_probability_falls_back_to_heuristic() -> None:
    heuristic = Evaluation("heuristic", utility=8.0, success_probability=0.95)
    risky = Evaluation(
        "rl-choice", utility=9.0, success_probability=0.40, safety_lower_bound=1.0
    )
    guard = PolicyGuard(PolicyGuardConfig(inference_timeout_s=0.2))
    try:
        result = guard.accept_or_fallback(
        ...
        assert result.used_fallback
        assert result.reason == "rl_success_probability_guard"
        assert result.selected.action_id == "heuristic"
    finally:
        guard.close()
