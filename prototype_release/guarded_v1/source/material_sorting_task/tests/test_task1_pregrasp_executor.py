...
class OpenPregraspControllerTests(unittest.TestCase):
    ...
    def test_optional_pregrasp_half_width_preserves_target_and_narrows_sweep(
        self,
    ) -> None:
        kdl = FakeKdl()
        controller = OpenPregraspController(
            kdl=kdl,
            pregrasp_half_width_m=0.140,
        )
        ...
        self.assertAlmostEqual(controller.pregrasp_half_width_m, 0.140, places=6)
        self.assertAlmostEqual(kdl.left[1, 3], 0.140, places=6)
        self.assertAlmostEqual(kdl.right[1, 3], -0.140, places=6)
        self.assertAlmostEqual(kdl.left[0, 3], 0.57, places=6)
    ...
    def test_optional_contact_center_backoff_makes_only_fore_aft_grasp_shallower(
        self,
    ) -> None:
        kdl = FakeKdl()
        controller = ContactGraspController(kdl=kdl, center_backoff_x=-0.010)
        ...
        self.assertAlmostEqual(kdl.left[0, 3], 0.660, places=6)
        self.assertAlmostEqual(kdl.left[1, 3], 0.118, places=6)
        self.assertAlmostEqual(kdl.right[1, 3], -0.118, places=6)
    ...
    def test_locked_side_allows_strong_stable_opposite_contact_latch(self) -> None:
        controller = ContactGraspController(kdl=FakeKdl())
        expected_slide = 1.32163718 - 0.854
        feedback = joint_states(slide=expected_slide)
        for tick in range(9):
        ...
        # The overall dual-arm pose is deliberately not settled.  A genuine
        # opposite-pad contact must still be accepted after the usual debounce
        # when it has firm effort, surface rotation and near-zero wrist speed.
        for tick in range(20):
            controller._update_wrist_compliance(
                controller._right_wrist,
                0.50 + tick * 0.10,
                position=0.06,
                velocity=0.0,
                effort=2.0,
        ...
        self.assertTrue(controller._right_wrist.contact_seen)
        self.assertTrue(controller._right_wrist.aligned)
        self.assertTrue(controller.bilateral_aligned)
