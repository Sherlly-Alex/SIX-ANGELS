...
from learning.isolated_inference import InferenceResult, SupervisorPoll
...
class SchedulerDecisionTests(unittest.TestCase):
    ...
    def test_isolated_shadow_never_changes_the_heuristic_action(self) -> None:
        class IsolatedWorker:
            ...
            def poll(self, *, expected_signature, now_s):
                del now_s
                if not self.ready:
            ...
            def submit(self, *, signature, observation, action_mask, now_s):
                del observation, action_mask, now_s
        ...
        service = SchedulerDecisionService(
            config=DecisionConfig(policy_mode="rl_shadow", candidate_stability_frames=1),
            isolated_policy_supervisor=worker,
        )
        try:
            first = service.decide(
                (candidate("high", 10.0), candidate("low", 1.0)), now_s=1.0
            )
            worker.ready = True
            second = service.decide(
                (candidate("high", 10.0), candidate("low", 1.0)), now_s=2.0
            )
            self.assertEqual(first.action_id, "high")
            self.assertEqual(second.action_id, "high")
            self.assertEqual(second.source, "heuristic")
            self.assertEqual(second.policy_suggestion.action_id, "low")
            self.assertEqual(second.policy_decision_reason, "accepted")
        finally:
            service.close()
        self.assertTrue(worker.closed)

    def test_isolated_shadow_stale_response_stays_heuristic(self) -> None:
        class StaleWorker:
            def poll(self, *, expected_signature, now_s):
                del expected_signature, now_s
        ...
        service = SchedulerDecisionService(
            config=DecisionConfig(policy_mode="rl_shadow", candidate_stability_frames=1),
            isolated_policy_supervisor=StaleWorker(),
        )
        try:
            outcome = service.decide(
                (candidate("high", 10.0), candidate("low", 1.0)), now_s=1.0
            ...
            self.assertEqual(outcome.action_id, "high")
            self.assertIsNone(outcome.policy_suggestion)
            self.assertEqual(outcome.policy_decision_reason, "inference_result_stale")
        finally:
            service.close()
