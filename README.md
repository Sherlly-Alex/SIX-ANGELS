...
局部建图是辅助功能，默认关闭，不替代全局 A*、状态机或 Heuristic。它分为两个开关：

- `MATERIAL_LOCAL_MAP=1`：计算并记录局部地图建议，但不改变机器人运动。
- `MATERIAL_LOCAL_MAP_APPLY=1`：在前一开关开启时，允许新鲜且有效的建议把 Task 2 的少数安全站位向外推远，并在 Task 2/3 运输导航中试验速度倍率；数据缺失、过期或异常时保持原参数。

当前 qzhRL 没有照抄压缩包中“安全检查后放大”的实现，而是把倍率前移到导航控制器
内部，使局部避障、速度/加速度限制和预测碰撞检查都看到最终候选速度。Task 1 与全部
放置链保持原控制路径；该实验功能仍默认关闭。

建议先只观测：

```bash
export MATERIAL_LOCAL_MAP=1
export MATERIAL_LOCAL_MAP_APPLY=0
bash "$PROJECT/material_sorting_task/scripts/competitionctl.sh" client "$RUN" heuristic
```
...
关闭两个变量即可完全回到已验证路径。详细设计与参数见
[局部建图策略](material_sorting_task/docs/LOCAL_MAP_STRATEGY.md)。
...
- [L1 彩色层可见性优化审计](material_sorting_task/docs/SHELF_L1_VISIBILITY_OPTIMIZATION_AUDIT.md)

当前货架识别还包含 L1 彩色层遮挡恢复：包装箱与显式空层证据仅用于触发一次补充
观察姿态，完整货架状态和任务二目标仍必须来自直接彩色箱 RGB-D 稳定观测。
