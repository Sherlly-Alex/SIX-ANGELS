...
class IntegratedExecutorWiringTests(unittest.TestCase):
    def test_task1_uses_slow_staged_shelf_release(self) -> None:
        ...
        # The release requirement is a 4 cm increase in bilateral hold radius.
    ...
    def test_task2_shelf_approach_allows_gaussian_render_latency(self) -> None:
        executor = Task2IntegratedExecutor(CompetitionTaskMemory())

        self.assertGreaterEqual(executor.SHELF_ARM_APPROACH_TIMEOUT_S, 45.0)
    ...
    def test_transfer_navigation_allows_gaussian_render_latency(self) -> None:
        motion = TransferMotion()

        self.assertGreaterEqual(motion.NAVIGATION_TIMEOUT_S, 180.0)
    ...
    def test_transfer_lateral_alignment_resets_timeout_for_each_motion(self) -> None:
        motion = TransferMotion()
        self.assertTrue(
            motion.begin_lateral_alignment(
                (-1.30, 1.00),
        ...
        # Rotation may legitimately use most of its bounded window.  Entering
        # lateral travel must start a fresh window instead of failing on the
        # next tick because the rotation consumed the shared timer.
        status, command, detail = motion.tick_lateral_alignment(
            _odom(-1.30, 0.85, math.pi / 2.0), 29.0
        )
        self.assertEqual(status, NavigationStatus.NAVIGATING)
        self.assertGreater(command[0], 0.0)
        self.assertIn("driving along shelf front", detail)

        status, command, detail = motion.tick_lateral_alignment(
            _odom(-1.30, 0.90, math.pi / 2.0), 31.0
        )
        self.assertEqual(status, NavigationStatus.NAVIGATING)
        self.assertGreater(command[0], 0.0)
        self.assertIn("driving along shelf front", detail)
