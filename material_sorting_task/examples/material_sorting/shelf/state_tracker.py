...
class ShelfStateTracker:
    ...
    def __init__(
        ...
        *,
        ...
        allow_l1_layout_packaging_fallback: bool = False,
    ) -> None:
        if window_size < 3:
        ...
        self.allow_l1_layout_packaging_fallback = bool(
            allow_l1_layout_packaging_fallback
        )
        ...
        self._layout_inferred_packaging: (
            tuple[int, tuple[float, float, float], float, int] | None
        ) = None
    ...
    def reset(self) -> None:
        ...
        self._layout_inferred_packaging = None
    ...
    def _resolved_packaging_evidence(
        ...
        if (
            self._stable_occupancy is not None
            and self._stable_occupancy[0] == inferred_layer
        ):
            return self._stable_occupancy

        # The carried task-1 box can completely hide the low packaging prop
        # from both YOLO and the L1 depth occupancy probe.  The official scene
        # still has exactly one coloured object, one packaging object and one
        # empty layer.  Permit a deliberately narrow recovery only after RGB-D
        # has explicitly confirmed the empty layer, the instructed colour is
        # stable, and their unique complement is L1.  Placement safety remains
        # governed by the explicit empty-layer confirmation; higher-layer or
        # ambiguous cases continue to fail closed.
        if not self.allow_l1_layout_packaging_fallback or inferred_layer != 1:
            return None
        colored_score = float(self._stable_colored[3])
        colored_count = int(self._stable_colored[4])
        empty_score = float(self._stable_empty[2])
        empty_count = int(self._stable_empty[3])
        self._layout_inferred_packaging = (
            inferred_layer,
            (
                float(self.geometry.shelf_xy[0]),
            ...
            min(colored_score, empty_score),
            min(colored_count, empty_count),
        )
        return self._layout_inferred_packaging
    ...
    def _packaging_vote_status(self) -> str:
        if self._layout_inferred_packaging is not None:
            layer, _center, _score, count = self._layout_inferred_packaging
            return f"layout-inferred:L{layer}({count})"
        if self._stable_packaging is not None:
