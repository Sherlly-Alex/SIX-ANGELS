...
class CompetitionClient(Node):
    ...
    def __init__(self) -> None:
        ...
        self.scheduler_rl_enabled = False
        ...
        if self.scheduler_mode == "v2":
            try:
                from scheduler.runtime_policy import build_v2_scheduler_stack
                ...
                stack = build_v2_scheduler_stack(event_log=event_log)
                decision_service = stack.decision_service
                candidate_provider = stack.candidate_provider
                self.scheduler_policy = stack.scheduler_policy
                self.scheduler_rl_enabled = bool(stack.rl_enabled)
                if stack.rl_load_error:
                    self.get_logger().error(
                        stack.rl_load_error + "; keeping V2 Heuristic"
                    )
                elif stack.loads_model:
                    self.get_logger().info(
                        "scheduler RL overlay enabled; policy_mode="
                        f"{stack.scheduler_policy}"
                    )
            ...
            try:
                event_log.emit(
                    ...
                    details={
                        ...
                        "rl_enabled": int(self.scheduler_rl_enabled),
