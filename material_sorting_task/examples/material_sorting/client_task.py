...
from task_orchestration import parse_gameinfo, sorted_instructions, startup_target_color
...
class CompetitionClient(Node):
    ...
    def _missing_inputs(self) -> list[str]:
        ...
        if self.execution_mode in {
            ...
            target_color = startup_target_color(
                self.instructions,
                self.referee_gameinfo,
            )
            if target_color and target_color not in self.target_observations:
