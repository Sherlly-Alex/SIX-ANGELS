...
from task_orchestration import referee_place_confirmed
...
class Task3IntegratedExecutor(Task1IntegratedExecutor):
    ...
    # The official RGB-D cuboid fit reports the white reference close to its
    # upper surface, so the pink centre can be only about 4 cm higher even
    # though it is physically on top.  Keep a positive separation gate while
    # accepting that measured representation.
    TASK3_TARGET_ABOVE_REFERENCE_Z_RANGE_M = (0.02, 0.35)
    TASK3_PICK_YAW = math.pi / 2.0
    TASK3_PICK_STANDOFF_M = 0.65
    # Calibrated observation stand for the fixed white cube, not a hard-coded
    # coloured-target position.  RGB-D at this pose still determines the pink
    # box centre used by grasping.
    TASK3_OBSERVE_STAND = (-0.54, 1.55)
    TASK3_TARGET_TIMEOUT_S = 45.0
    ...
    # Bounded only for the post-release safety actions.  Final navigation to
    # the end zone has its own TransferMotion timeout and must not consume
    # this same budget.
    TASK3_RETURN_SEQUENCE_TIMEOUT_S = 120.0
    TASK3_VERIFY_PLACE_TIMEOUT_S = 3.0
    ...
    def __init__(self, memory) -> None:
        ...
        self._task3_using_observation_stand = False
        self._task3_observe_reached_s: float | None = None
    ...
    def reset(self) -> None:
        ...
        self._task3_using_observation_stand = False
        self._task3_observe_reached_s = None
    ...
    def tick(self, stage: TaskStage, context: ExecutionContext) -> StageResult:
        ...
        if stage is TaskStage.VERIFY_PLACE:
            # The official referee can only confirm task 3 after the released
            # box has been pushed to its final shelf position.  Defer that
            # check to RETURN_TO_END, immediately after the bounded push and
            # retreat, instead of deadlocking before the push starts.
            return StageResult.succeeded(
                "task 3 release complete; official step=place verification "
                "will follow the post-release push",
                arm_command=self._held_arm_command,
            )
    ...
    def _top_box_observation(self, context: ExecutionContext):
        ...
        if int(context.attempt) <= 1:
            ...
            if reference is None:
                if (
                    not self._task3_using_observation_stand
                    and self._task3_source_anchor_world is None
                ):
                    return color, None, "waiting for current white-cube RGB-D reference"
                # At the calibrated observation stand the pink box can fully
                # occlude the white cube class.  Once the target has been
                # anchored there, the white reference can remain invisible at
                # the closer pick stand.  The coloured target itself is still
                # a fresh official RGB-D observation, so keep tracking it
                # against the locked anchor rather than deadlocking.
                return color, observation, "ready; using locked live target anchor"
    ...
    def _source_center_from_observation(
        ...
        *,
        ...
        if first_attempt:
            if reference is None:
                if self._task3_using_observation_stand:
    ...
    def _tick_task3_navigate_to_pick(self, context: ExecutionContext) -> StageResult:
        ...
        if self._goal is None:
            ...
            if self._task3_source_anchor_world is not None:
                target_x, target_y, _target_z = self._task3_source_anchor_world
                goal_x = float(target_x)
                goal_y = float(target_y) - self.TASK3_PICK_STANDOFF_M
                source_tag = "task3_top_box_rgbd"
                self._coarse_target_world = self._task3_source_anchor_world
                self._task3_using_observation_stand = False
            else:
                goal_x, goal_y = self.TASK3_OBSERVE_STAND
                source_tag = "task3_white_cube_observation_stand"
                self._task3_using_observation_stand = True
        ...
        if status is NavigationStatus.GOAL_REACHED:
            if self._task3_using_observation_stand:
                if self._task3_observe_reached_s is None:
                    self._task3_observe_reached_s = float(context.now_s)
                if self._task3_source_anchor_world is None:
                    elapsed = max(
                        0.0,
                        float(context.now_s) - self._task3_observe_reached_s,
                    )
                    if elapsed >= self.TASK3_TARGET_TIMEOUT_S:
                        return StageResult.blocked(
                            f"task 3 timed out locking {color} at the white-cube "
                            f"observation stand after {elapsed:.1f}s: {detail}"
                        )
                    return StageResult.running(
                        "task 3 reached the white-cube observation stand and is "
                        f"locking the live {color} centre before the final approach; "
                        f"{detail}"
                    )
                # The observation stand is intentionally outside arm reach.
                # Re-plan once from the live RGB-D centre to the actual pick
                # stand; only that second goal may advance to arm pregrasp.
                self._navigation.reset()
                self._goal = None
                self._task3_using_observation_stand = False
                return StageResult.running(
                    "task 3 locked the live top-box centre at the observation "
                    "stand; planning the final object-relative pick approach"
                )
            return StageResult.succeeded(
    ...
    def _ensure_task3_place_target(self, context: ExecutionContext) -> None:
        ...
        # and the shelf layer remembered from task 1 when that cache is
        # available.  A Client restart does not restart the official Server,
        # so task 3 may legitimately resume without task 1's process-local
        # cache.  In that case the formal task-3 instruction remains the
        # authoritative placement interface.
        try:
            state = self._memory.require_shelf_state()
            packaging_center = self._memory.require_task3_packaging_box_center()
        except RuntimeError:
            state = None
            packaging_center = None
        ...
        layer_candidates = tuple(
            (
                layer,
                self._shelf_tracker.geometry.object_center_z_on_board(
                    layer, half_z=0.095
                ),
            )
            for layer in range(1, len(self._shelf_tracker.geometry.board_surfaces_z) + 1)
        )
        if state is not None and packaging_center is not None:
            relative_xy = (
            ...
            white_layer = int(state.white_obstacle_layer)
        else:
            left_separation = self.TASK3_MIN_PROP_CENTER_SEPARATION_M
            white_layer = min(
                layer_candidates,
                key=lambda item: abs(scoring_target[2] - item[1]),
            )[0]
        expected_z = self._shelf_tracker.geometry.object_center_z_on_board(
            white_layer, half_z=0.095
        )
        ...
        self._task3_white_layer = white_layer
    ...
    def _tick_task3_transport(self, context: ExecutionContext) -> StageResult:
        ...
        if self._phase == "task3_turn_ccw_to_shelf":
            if self._shelf_scan_stand is None:
                try:
                    packaging_y = float(
                        self._memory.require_task3_packaging_box_center()[1]
                    )
                except RuntimeError:
                    assert self._task3_scoring_place is not None
                    packaging_y = float(self._task3_scoring_place[1])
                self._shelf_scan_stand = shelf_observation_stand(
                    self._held_center_base,
                    shelf_front_x=self.SHELF_FRONT_X,
                    shelf_y=max(0.58, min(0.98, packaging_y)),
    ...
    def _tick_task3_place(self, context: ExecutionContext) -> StageResult:
        ...
        if self._phase == "release_support_settle":
            ...
            if elapsed < self.RELEASE_SUPPORT_SETTLE_S:
                return StageResult.running(
                    "task 3 holding shelf contact before unloading preload "
                    f"({elapsed:.2f}/{self.RELEASE_SUPPORT_SETTLE_S:.2f}s)",
                    arm_command=self._held_arm_command,
                )
            self._phase = "release"
            self._phase_started_s = float(context.now_s)
        ...
        if self._phase == "release":
            ...
            if float(context.now_s) - self._phase_started_s >= self.PLACE_TIMEOUT_S:
                self._report_place_time(context, "release_timeout")
                return StageResult.blocked(
    ...
    def _tick_task3_return_to_end(self, context: ExecutionContext) -> StageResult:
        ...
        elapsed = max(0.0, float(context.now_s) - self._stage_started_s)
        # Once the arms are retracted and the spine is raised, delegate to
        # the inherited checked end-zone navigation.  Its navigation timeout
        # is independent; applying the post-release budget here used to stop
        # the robot just before/inside the final return route.
        if (
            self._phase != "navigate_end"
            and elapsed >= self.TASK3_RETURN_SEQUENCE_TIMEOUT_S
        ):
            return StageResult.blocked(
        ...
        if self._phase == "task3_post_push_retreat":
            ...
            self._phase = "task3_verify_place"
            self._phase_started_s = float(context.now_s)
            self._motion_started = False
            self._transfer.reset()
            self._arm_retract.reset()

        if self._phase == "task3_verify_place":
            if referee_place_confirmed(context.referee_gameinfo, context.task_index):
                self._report_place_time(context, "confirmed")
                ...
                if verify_elapsed >= self.TASK3_VERIFY_PLACE_TIMEOUT_S:
                    self._report_place_time(context, "not_confirmed")
                    return StageResult.blocked(
                        "task 3 post-push placement was not confirmed by the "
                        "official referee within "
                        f"{self.TASK3_VERIFY_PLACE_TIMEOUT_S:.1f}s; "
                        f"referee_step={context.referee_gameinfo.get('step', '-')}",
                        arm_command=self._held_arm_command,
                    )
                return StageResult.running(
                    "task 3 waiting for official referee step=place after "
                    f"post-release push ({verify_elapsed:.1f}/"
                    f"{self.TASK3_VERIFY_PLACE_TIMEOUT_S:.1f}s)",
                    arm_command=self._held_arm_command,
                )
