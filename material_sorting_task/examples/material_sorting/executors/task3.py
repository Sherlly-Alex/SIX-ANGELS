...
class Task3IntegratedExecutor(Task1IntegratedExecutor):
    ...
    # On the first attempt the coloured 19 cm box is stacked on the measured
    # 17 cm-high material box.  A far-view side-face cloud can otherwise be
    # mistaken for a horizontal face and move the fitted centre down by one
    # coloured-box half-height (9.5 cm).  Preserve trustworthy RGB-D heights,
    # but recover that discrete ambiguity from the live white-box reference.
    TASK3_STACKED_CENTER_DZ_M = 0.085 + 0.095
    TASK3_STACKED_Z_MAX_DEVIATION_M = 0.05
    ...
    def _update_table_reference(self, context: ExecutionContext) -> None:
        # Freeze the same reference used to create the pick stand.  Close-view
        # arm occlusion must not move the height guard while navigating.
    ...
    def _task3_grasp_z(self, observed_z: float, *, first_attempt: bool) -> float:
        observed_z = float(observed_z)
        reference = self._task3_table_reference_world
        if not first_attempt or reference is None:
            self._task3_target_height_detail = ""
            return observed_z

        expected_z = float(reference[2]) + self.TASK3_STACKED_CENTER_DZ_M
        error_m = observed_z - expected_z
        if abs(error_m) <= self.TASK3_STACKED_Z_MAX_DEVIATION_M:
            self._task3_target_height_detail = ""
            return observed_z

        self._task3_target_height_detail = (
            "task 3 stack-height guard corrected RGB-D center "
            f"z={observed_z:.3f}m to {expected_z:.3f}m "
            f"from white-box reference z={float(reference[2]):.3f}m"
        )
        return expected_z
    ...
    def _source_center_from_observation(
        ...
        *,
        ...
        if first_attempt:
            ...
            # from RGB-D unless it has the discrete half-height ambiguity seen
            # on a partially visible side face.
            grasp_z = self._task3_grasp_z(observed_z, first_attempt=True)
            return float(reference[0]), float(reference[1]), grasp_z
    ...
    def _tick_task3_acquire_target(self, context: ExecutionContext) -> StageResult:
        ...
        if self._task3_target_center is not None and observation is None:
            height_detail = (
                f"; {self._task3_target_height_detail}"
                if self._task3_target_height_detail
                else ""
            )
            return StageResult.succeeded(
                ...
                f"orientation={self.SOURCE_ORIENTATION}{height_detail}"
            )
        estimate = self._task3_target_tracker.update(
            observation,
            now_s=context.now_s,
