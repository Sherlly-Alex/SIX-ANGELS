...
class Task1IntegratedExecutor(Task1LiftExecutor):
    ...
    def enter_stage(self, stage: TaskStage, context: ExecutionContext) -> None:
        ...
        if stage is TaskStage.TRANSPORT:
            ...
            self._slide_hold.reset()
            self._transfer.reset()
            # Transport observations can be partial or show a desktop object
            # before the shelf is fully in view.  The dedicated shelf scan is
            # the only observation window allowed to populate cross-task
            # memory, so start it with a fresh fusion buffer.
            self._shelf_tracker.reset()
            self._shelf_state = None
    ...
    def _tick_align_for_place(self, context: ExecutionContext) -> StageResult:
        ...
        if self._phase == "scan_shelf":
            ...
            # Do not reuse a transport-time result here.  This scan starts
            # with a reset tracker in ``enter_stage`` and must supply the
            # shelf state that task 2/3 later consume.
