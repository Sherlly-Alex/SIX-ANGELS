...
class Task2IntegratedExecutor(Task1LiftExecutor):
    ...
    # Gaussian rendering can make this 0.4 m closed-loop segment take just
    # over 30 wall-clock seconds even while odometry is steadily converging.
    # Collision monitoring remains active on every tick; this only prevents a
    # healthy final few centimetres from being mistaken for a stalled motion.
    SHELF_ARM_APPROACH_TIMEOUT_S = 45.0
    # Explicitly bound both shelf lateral-alignment passes.  The default
    # TransferMotion watchdog is 30 s, which is too short under Gaussian
    # rendering even while yaw and row errors are steadily decreasing.
    SHELF_LATERAL_ALIGNMENT_TIMEOUT_S = 60.0
    ...
    VERIFY_PLACE_TIMEOUT_S = 3.0
    RELEASE_SUPPORT_SETTLE_S = 0.40
    RELEASE_SPREAD_M = 0.040
    ARM_RETRACT_TIMEOUT_S = 15.0
    # The Gaussian-rendered simulator can slow feedback enough that the final
    # table-entry segment is still making progress when 30 s expires.  Keep
    # the same bounded watchdog, but allow the segment to finish safely.
    TRANSPORT_SEGMENT_TIMEOUT_S = 60.0
    ...
    def __init__(self, memory: CompetitionTaskMemory) -> None:
        ...
        self._place_started_s: float | None = None
        self._place_time_reported = False
    ...
    def enter_stage(self, stage: TaskStage, context: ExecutionContext) -> None:
        ...
        if stage is TaskStage.NAVIGATE_TO_PICK:
            ...
            self._phase = "lower"
            self._phase_started_s = float(context.now_s)
            self._place_started_s = float(context.now_s)
            self._place_time_reported = False
        ...
        if stage is TaskStage.VERIFY_PLACE:
            ...
            if referee_place_confirmed(context.referee_gameinfo, context.task_index):
                ...
                return StageResult.succeeded(
                    "task 2 table placement confirmed by official referee step=place",
                    arm_command=self._held_arm_command,
                )
            if elapsed >= self.VERIFY_PLACE_TIMEOUT_S:
                self._report_place_time(context, "not_confirmed")
                return StageResult.blocked(
                    "task 2 table placement was not confirmed by the official "
                    f"referee within {self.VERIFY_PLACE_TIMEOUT_S:.1f}s; "
                    f"referee_step={context.referee_gameinfo.get('step', '-')}",
                    arm_command=self._held_arm_command,
                )
            return StageResult.running(
                "task 2 holding release pose until official referee reports "
                f"step=place ({elapsed:.1f}/{self.VERIFY_PLACE_TIMEOUT_S:.1f}s)",
                arm_command=self._held_arm_command,
            )
    ...
    def _tick_place(self, context: ExecutionContext) -> StageResult:
        ...
        if self._phase == "lower":
            ...
            result = self._tick_slide(
                context,
                "lowering box onto original table point",
        ...
        if self._phase == "support_settle":
            ...
            if elapsed < self.RELEASE_SUPPORT_SETTLE_S:
                return StageResult.running(
                    "task 2 holding contact so the table carries the load "
                ...
                try:
                    ...
                    if not math.isfinite(held_half_width) or held_half_width <= 0.0:
                        raise PregraspPlanningError(
                            "held half-width is invalid before table release"
                        )
                    self._held_arm_command = self._release.plan(
                        self._place_world,
                        context.odometry,
                        context.joint_states,
                        half_width=held_half_width + self.RELEASE_SPREAD_M,
                    )
                except (PregraspInputError, PregraspPlanningError) as exc:
                    return StageResult.blocked(
        ...
        if not reached:
            ...
            if elapsed >= timeout_s:
                if self.active_stage is TaskStage.PLACE:
                    self._report_place_time(context, "place_timeout")
                return StageResult.blocked(
    ...
    def _report_place_time(self, context: ExecutionContext, result: str) -> None:
        if self._place_time_reported or self._place_started_s is None:
            return
        elapsed = max(0.0, float(context.now_s) - self._place_started_s)
        print(
            f"[PLACE_TIME] task=2 attempt={context.attempt} "
            f"duration_s={elapsed:.3f} result={result}",
            flush=True,
        )
        self._place_time_reported = True
