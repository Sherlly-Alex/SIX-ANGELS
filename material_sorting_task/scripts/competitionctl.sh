...
EXTERNAL_LOCAL_MAP="${MATERIAL_LOCAL_MAP-}"
EXTERNAL_LOCAL_MAP_APPLY="${MATERIAL_LOCAL_MAP_APPLY-}"
EXTERNAL_LOCAL_MAP_HZ="${MATERIAL_LOCAL_MAP_HZ-}"
EXTERNAL_LOCAL_MAP_RES="${MATERIAL_LOCAL_MAP_RES-}"
EXTERNAL_LOCAL_MAP_MAX_AGE_S="${MATERIAL_LOCAL_MAP_MAX_AGE_S-}"
EXTERNAL_LOCAL_MAP_MIN_HITS="${MATERIAL_LOCAL_MAP_MIN_HITS-}"
EXTERNAL_LOCAL_MAP_STRIDE="${MATERIAL_LOCAL_MAP_STRIDE-}"
EXTERNAL_LOCAL_MAP_SPEED_MODE="${MATERIAL_LOCAL_MAP_SPEED_MODE-}"
EXTERNAL_LOCAL_MAP_CLEAR_BOOST="${MATERIAL_LOCAL_MAP_CLEAR_BOOST-}"
...
[[ -n "$EXTERNAL_LOCAL_MAP" ]] && MATERIAL_LOCAL_MAP="$EXTERNAL_LOCAL_MAP"
[[ -n "$EXTERNAL_LOCAL_MAP_APPLY" ]] && \
  MATERIAL_LOCAL_MAP_APPLY="$EXTERNAL_LOCAL_MAP_APPLY"
[[ -n "$EXTERNAL_LOCAL_MAP_HZ" ]] && MATERIAL_LOCAL_MAP_HZ="$EXTERNAL_LOCAL_MAP_HZ"
[[ -n "$EXTERNAL_LOCAL_MAP_RES" ]] && MATERIAL_LOCAL_MAP_RES="$EXTERNAL_LOCAL_MAP_RES"
[[ -n "$EXTERNAL_LOCAL_MAP_MAX_AGE_S" ]] && \
  MATERIAL_LOCAL_MAP_MAX_AGE_S="$EXTERNAL_LOCAL_MAP_MAX_AGE_S"
[[ -n "$EXTERNAL_LOCAL_MAP_MIN_HITS" ]] && \
  MATERIAL_LOCAL_MAP_MIN_HITS="$EXTERNAL_LOCAL_MAP_MIN_HITS"
[[ -n "$EXTERNAL_LOCAL_MAP_STRIDE" ]] && \
  MATERIAL_LOCAL_MAP_STRIDE="$EXTERNAL_LOCAL_MAP_STRIDE"
[[ -n "$EXTERNAL_LOCAL_MAP_SPEED_MODE" ]] && \
  MATERIAL_LOCAL_MAP_SPEED_MODE="$EXTERNAL_LOCAL_MAP_SPEED_MODE"
[[ -n "$EXTERNAL_LOCAL_MAP_CLEAR_BOOST" ]] && \
  MATERIAL_LOCAL_MAP_CLEAR_BOOST="$EXTERNAL_LOCAL_MAP_CLEAR_BOOST"
...
run_client() {
  ...
  exec_env=(
    ...
    -e MATERIAL_LOCAL_MAP="${MATERIAL_LOCAL_MAP:-0}"
    -e MATERIAL_LOCAL_MAP_APPLY="${MATERIAL_LOCAL_MAP_APPLY:-0}"
    -e MATERIAL_LOCAL_MAP_HZ="${MATERIAL_LOCAL_MAP_HZ:-0.5}"
    -e MATERIAL_LOCAL_MAP_RES="${MATERIAL_LOCAL_MAP_RES:-0.05}"
    -e MATERIAL_LOCAL_MAP_MAX_AGE_S="${MATERIAL_LOCAL_MAP_MAX_AGE_S:-8.0}"
    -e MATERIAL_LOCAL_MAP_MIN_HITS="${MATERIAL_LOCAL_MAP_MIN_HITS:-2}"
    -e MATERIAL_LOCAL_MAP_STRIDE="${MATERIAL_LOCAL_MAP_STRIDE:-8}"
    -e MATERIAL_LOCAL_MAP_SPEED_MODE="${MATERIAL_LOCAL_MAP_SPEED_MODE:-boost_only}"
    -e MATERIAL_LOCAL_MAP_CLEAR_BOOST="${MATERIAL_LOCAL_MAP_CLEAR_BOOST:-1.40}"
  )
  if [[ "$mode" == "guarded" || "$mode" == "shadow" ]]; then
    exec_env+=(
