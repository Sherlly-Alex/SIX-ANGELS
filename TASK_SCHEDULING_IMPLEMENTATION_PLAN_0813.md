...
R2 远程诊断进一步给出实际因果链：transport sidecar 在固定 0.70 m 桌边后退尚未完成时，曾先
应用中心站位，随后按当时仍靠北的 live pose 将目标切换为向南 0.08 m 的 left 站位
`(-0.9995, 0.6185, pi)`；真正开始 A* 时底盘已经完成向南后退，实测货箱扫掠对南墙净空变为
`-0.006 m`。因此问题不是 2 cm 阈值，而是“候选评估位姿”和“动作生效位姿”跨越了一个固定运动。

最终约束为：measured-carry 开启时，桌边后退期间收到的 transport 候选只记 `audit_only`，不得
提前改写后退后的 A* 目标；后退完成后，执行器从真实起点对名义中心站位运行同一个实测包络门，
通过才启动导航。其他阶段候选应用、Heuristic 选择、2 cm 净空、逐周期命令检查均保持不变。
