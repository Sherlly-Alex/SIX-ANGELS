...
class Task3IntegratedExecutor(Task1IntegratedExecutor):
    ...
    TASK3_PUSHER_HALF_WIDTH_M = 0.085
    TASK3_FINAL_CENTER_X = -2.680
    TASK3_PUSH_X_TOLERANCE_M = 0.025
    TASK3_PUSH_OVERSHOOT_M = 0.020
    TASK3_PUSH_MAX_DISTANCE_M = 0.240
    TASK3_PUSH_TIMEOUT_S = 24.0
    ...
    def __init__(self, memory: CompetitionTaskMemory) -> None:
        ...
        self._release = Task3GripperOnlyReleaseController()
    ...
    def _tick_task3_place(self, context: ExecutionContext) -> StageResult:
        ...
        if phase == "parent_release":
            result = super()._tick_place(context)
            if result.status is not StageStatus.SUCCEEDED:
                # The validated Task3 runner opens for a fixed delay and then
                # backs out. Do not wait for both arms to attain a deep-shelf
                # IK pose after the grippers have opened and the box has
                # settled on the board.
                release_settled = (
                    self._phase == "release"
                    and self._release.planned
                    and float(context.now_s) - self._phase_started_s
                    >= self.TASK3_RELEASE_SETTLE_S
                )
                if not release_settled:
                    return result
        ...
        if self._task3_place_phase == "prepare_open_pusher":
            try:
                if not self._task3_pusher.planned:
                    self._held_arm_command = self._task3_pusher.plan(
                        self._place_world,
                        context.odometry,
                        context.joint_states,
            ...
            self._task3_place_phase = "visual_push"
            self._task3_push_started_s = float(context.now_s)
            self._task3_push_start_x = pose[0]
            self._task3_push_last_seen_s = float(context.now_s)
            self._task3_push_confirm_count = 0
            return StageResult.running(
                "Task3 open centre pusher ready; starting RGB-D closed-loop push",
                arm_command=command,
            )
        ...
        if self._task3_place_phase == "visual_push":
            ...
            error_x = center_x - self.TASK3_FINAL_CENTER_X
            ...
            if self._task3_push_confirm_count >= 3:
                return StageResult.succeeded(
                    ...
                    f"target={self.TASK3_FINAL_CENTER_X:.3f}",
                    arm_command=self._held_arm_command,
                )
