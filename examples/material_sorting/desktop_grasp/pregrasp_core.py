...
class OpenPregraspController:
    ...
    def update(
        ...
        stable_now = (
            ...
            and max_velocity <= (
                self.CONTACT_VEL_TOL
                if isinstance(self, ContactGraspController)
                else FEEDBACK_VEL_TOL
            )
        )
        reached = False
        if stable_now:
...
class ContactGraspController(OpenPregraspController):
    ...
    # Physical box contact introduces small joint-velocity jitter.
    # Allow the contact pose to settle without relaxing normal pregrasp/lift.
    CONTACT_VEL_TOL = 0.03
